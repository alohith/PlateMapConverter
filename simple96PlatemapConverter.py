...
def long2square(df: pd.DataFrame, col: str, index: str) -> pd.DataFrame:
    ...
    wellLetter = [i for i in ascii_uppercase[:8]]
    wellNumber = [i for i in range(1, 13)]
...
class CommandLine:
    def __init__(self, inOpts=None) -> None:
        ...
        self.parser = argparse.ArgumentParser(
            description="A simple 96 platemap converter",
            epilog="can convert from array form to square form and vice versa",
            prog="simple96PlatermapConverter.py",
            add_help=True,
            prefix_chars="-",
        ...
        self.parser.add_argument(
            ...
            nargs="?",
            required=True,
            help="The 'index' column of your platemap (i.e. 96_Well)",
            type=str,
        )
...
def isSquare(df: pd.DataFrame) -> bool:
    if len(df.columns) == 12 and len(df.index) == 8:  # 16 * 24 = 384
        try:
            [
                int(i)
