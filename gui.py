...
def root_window():
    ...
    nw_frame.grid(
        column=0, columnspan=2, row=0, rowspan=1, padx=10, pady=10, sticky="nsew"
    )
    ...
    sw_frame.grid(
        column=0, columnspan=1, row=1, rowspan=1, padx=10, pady=10, sticky="nsew"
    )
    se_frame = tk.LabelFrame(main_frame, padx=10, pady=10)
    se_frame.grid(
        column=1, columnspan=1, row=1, rowspan=1, padx=10, pady=10, sticky="nsew"
    )

    tk.Label(nw_frame, text="Input / Output").grid(
        column=0, columnspan=1, row=0, rowspan=1, padx=10, pady=10, sticky="w"
    )
    ...
    def input_file_prompt():
        input_file_name = filedialog.askopenfile(
            initialdir="your directory path",
            title="Input File",
            filetypes=(("csv files", "*.csv"), ("all files", "*.*")),
        )

    tk.Button(nw_frame, text="Input File", command=input_file_prompt, width=20).grid(
        column=0, columnspan=1, row=1, rowspan=1, padx=10, pady=10, sticky="w"
    )
    ...
    output_label.grid(
        column=0, columnspan=1, row=2, rowspan=1, padx=10, pady=10, sticky="w"
    )

    def output_file_prompt():
        output_file_name = filedialog.askopenfile(
            initialdir="your directory path",
            title="Output File",
            filetypes=(("csv files", "*.csv"), ("all files", "*.*")),
        )

    output_text = tk.Entry(nw_frame, width=50).grid(
        column=0, columnspan=1, row=3, rowspan=1, padx=10, pady=10, sticky="w"
    )

    tk.Label(sw_frame, text="96 Well Options").grid(
        column=0, columnspan=1, row=0, rowspan=1, padx=10, pady=10, sticky="w"
    )
    ...
    ir0 = tk.Radiobutton(
        sw_frame, text="96 Square <-> 96 Long", variable=input_radio_var, value=0
    )
    ...
    tk.Label(se_frame, text="384 Well Options").grid(
        column=0, columnspan=1, row=0, rowspan=1, padx=10, pady=10, sticky="w"
    )
    ...
    or0 = tk.Radiobutton(
        se_frame, text="384 Square <-> 384 Long", variable=output_radio_var, value=0
    )
    ...
    or2 = tk.Radiobutton(
        se_frame, text="384 -> 96 x 4Q", variable=output_radio_var, value=1
    )
